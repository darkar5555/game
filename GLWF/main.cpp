...
int main( )
{
    ...
    bool res = loadOBJ("resources/obj/Lamborghini_Aventador.obj", vertices2, uvs, normals);
...
//    GLuint probando = Box::LoadBox();
    ...
    // Diffuse map
    image = SOIL_load_image( "resources/images/image2.png", &imageWidth, &imageHeight, 0, SOIL_LOAD_RGB );
...
bool loadOBJ(const char* path,vector<vec3> & out_vertices,vector<vec2> & out_uvs,vector<vec3> & out_normals)
{
    ...
    while( 1 )
    {
        ...
        else if(strcmp(lineHeader,"f")==0)
        {
            ...
            int matches = fscanf(file,"%d/%d/%d %d/%d/%d %d/%d/%d\n",&vertexIndex[0],&uvIndex,&normalIndex);
